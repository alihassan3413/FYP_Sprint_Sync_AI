  ...
  <div class="flex min-w-0 items-center gap-3">
    ...
    <div class="min-w-0">
      <div class="flex items-center gap-2">
        <span
          class="truncate text-[13.5px] font-medium text-foreground"
          :class="member.status === 'pending' && 'italic text-muted-foreground'"
        >
          {{ member.status === 'pending' ? member.email : member.name }}
        </span>
        <span
          v-if="member.is_self"
          class="rounded bg-violet-100 px-1.5 py-px text-[10px] font-semibold uppercase tracking-wider text-violet-700 dark:bg-violet-500/20 dark:text-violet-300"
        >
          You
        </span>
      </div>
