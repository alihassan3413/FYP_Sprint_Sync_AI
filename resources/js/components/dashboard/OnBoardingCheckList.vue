...
const remainingMinutes = computed(() => props.steps.filter((s) => !s.done).reduce((sum, s) => sum + (s.minutes ?? 0), 0));

const allDone = computed(() => completedCount.value === totalCount.value);

/** First incomplete step — gets the primary CTA */
const nextStepKey = computed(() => props.steps.find((s) => !s.done)?.key ?? null);
</script>
...
<template>
    <div class="bg-card overflow-hidden rounded-xl border shadow-sm">
            ...
            <div class="flex items-start justify-between gap-4">
                <div class="min-w-0">
                    ...
                    <p class="text-muted-foreground mt-1 text-xs">
                        <template v-if="allDone"> Every step complete · You're ready to ship. </template>
                        <template v-else>
        ...
        <ul class="px-2 pt-3 pb-2 sm:px-3 sm:pb-3">
            <li v-for="step in steps" :key="step.key">
                <div
                    class="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors"
                    :class="!step.done && step.key === nextStepKey ? 'bg-amber-50/50 dark:bg-amber-500/[0.05]' : 'hover:bg-muted/40'"
                >
                    <!-- Status indicator -->
                    <span
                        v-if="step.done"
                    ...
                    <span
                        ...
                        :class="step.key === nextStepKey ? 'border-amber-500 dark:border-amber-400' : 'border-muted-foreground/30'"
                        aria-label="Incomplete"
                    />

                    <!-- Step content -->
                    <div class="min-w-0 flex-1">
                        <p class="text-[13px] leading-tight font-medium" :class="step.done && 'text-muted-foreground line-through'">
                            {{ step.title }}
                        </p>
                        <p v-if="step.done && step.doneCaption" class="text-muted-foreground mt-0.5 text-[11.5px]">
                            {{ step.doneCaption }}
                        </p>
                        <p v-else-if="step.description" class="text-muted-foreground mt-0.5 text-[11.5px]">
                            {{ step.description }}
                        </p>
                    </div>

                    <!-- CTA — only on the next incomplete step -->
                    <Button v-if="!step.done && step.key === nextStepKey && step.href" as-child size="sm" class="h-7 gap-1 px-2.5 text-xs">
                        <Link :href="step.href">
                            Start
