...
<template>
  <div class="rounded-xl border bg-card p-5 shadow-sm">
    ...
    <div v-if="members.length === 0" class="py-4 text-center text-xs text-muted-foreground">
    ...
    <ul v-else class="space-y-2.5">
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          <p class="truncate text-[12.5px] font-medium leading-tight">
            {{ m.name }}
            <span
              v-if="m.is_self"
              class="ml-0.5 text-[10.5px] font-medium text-muted-foreground"
            >
              · you
            </span>
          </p>
    ...
    <Link
      v-if="overflow > 0"
      :href="viewAllHref"
      class="mt-3 inline-flex items-center text-[11.5px] font-medium text-muted-foreground hover:text-foreground"
    >
      +{{ overflow }} more →
    </Link>
  </div>
